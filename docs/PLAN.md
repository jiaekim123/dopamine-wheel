...
> **현재 상태**: **Phase 1~5 + v1.4.1 + v1.4.2 + v1.5 (A1~A7) + 컴포넌트 통합 테스트 + v2.2 (야생 더비) + v2.3 (동물 밸런스) + v2.3.1 (메인 이모지) + v2.4 (폭탄 fuse 랜덤) + v2.4.1 (폭탄 fuse 2~5s) 완료**, Phase 6 (반응형 + 배포) 대기
...
- ✅ 시네마틱 카메라 / 야생 더비(구 경주마) 16명 확장 — v2 V1+V4
- ✅ 부스터 타일 (야생 더비) — v2 V3
- ✅ 추첨기 가속 페이크 (로또) — v2 V5
- ✅ 룰렛 중간 기믹 (점프대) — v2 V6, v2.1로 무작위 배치
- ✅ 야생 더비 동물 6종 + quirk + 부스터/장애물 — v2.2
- ✅ 야생 더비 동물 밸런스 (positive quirk 6종 + baseSpeed 재조정) — v2.3
- ✅ 야생 더비 메인 이모지 = 참가자 본인 이모지 — v2.3.1
...
|---|---|---|
| 야생 더비 (Coral) | 트랙 라인 / 결승선 | 트랙 lane 사이 가는 네온 핑크 라인 / 결승 통과 시 네온 핑크 폭발 |
...
### 게임명 / 메타 변경
- 이름: `경주마 / Horse Race` → `야생 더비 / Wild Derby`
...
### `lib/games.js`
- horse 카드 `name: '야생 더비'`, `nameEn: 'Wild Derby'`, `emoji: '🏁'`, description 갱신.
...
`경주마` 텍스트 참조 갱신:
- `src/App.test.jsx`, `src/components/GameCard.test.jsx`, `src/components/GameCardGrid.test.jsx` — `야생 더비`로 일괄 변경.
...
## v2.3.1 — 야생 더비 메인 이모지 변경 ✅

> **사용자 요청**: "야생더비에 달리는 이모지는 그냥 본인 캐릭터의 이모지가 좋겠어. 각 동물 캐릭터 말고. 헤깔릴듯?"
